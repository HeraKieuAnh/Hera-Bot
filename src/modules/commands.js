    ...
    bot.commands = {
                ...
                if(voicechannel && voicechannel.type == 'voice') {
                    voicechannel.join()
                        .then(connection => {
                            ...
                            resolve(connection);
                            msg.channel.sendMessage(`:speaking_head: Da tham gia phong: ${voicechannel.name}`);
        ...
        leave: msg => {
            ...
            bot.client.voiceConnections.every(connection => {
                ...
                msg.channel.sendMessage(`:mute: Da thoat phong: ${connection.channel.name}`); 
            });
        ...
        pause: msg => {
            ...
            if(track && track.dispatcher) {
                ...
                msg.channel.sendMessage(`:pause_button: "${track.title}" da dung`);
            }
        },
        ...
        resume: msg => {
            ...
            if(track && track.dispatcher) {
                ...
                msg.channel.sendMessage(`:play_pause: "${track.title}" da tiep tuc`);
            }
        },
        ...
        youtube: msg => {
            ...
            __.all(targets, target => {
                ...
                bot.jukebox.info(track, msg, (err, info) => {
                    ...
                    msg.channel.sendMessage(`:heavy_plus_sign: Youtube da duoc them: "${track.title}" @ #${bot.queue.indexOf(track) + 1}`);
                });
            });
        },
            ...
            if(parts.length > 0 && songtypes.indexOf(type) > -1) {
                ...
                __.all(targets, target => {
                    ...
                    bot.jukebox.info(track, msg, (err, info) => {
                        ...
                        msg.channel.sendMessage(`:heavy_plus_sign: Bao gom: "${track.title}" @ #${bot.queue.indexOf(track) + 1}`);
                    });
                });
            } else {
                msg.channel.sendMessage('Duong link khong hop le: "{0}enqueue youtube:https://www.youtube.com/watch?v=dQw4w9WgXcQ"'
        ...
        skip: msg => {
            ...
            if(track && track.dispatcher && msg && msg.channel) {
                ...
                msg.channel.sendMessage(`:fast_forward: "${track.title}" da bo qua`);
            }
        },
        ...
        stop: msg => {
            ...
            if(track && track.dispatcher && msg && msg.channel) {
                ...
                track.dispatcher.end();
                bot.paused = false;
                msg.channel.sendMessage(`:stop_button: "${track.title}" da dung`);
            }
        },
        ...
        list: msg => {
            ...
            if(list.length > 0)
                ...
                msg.channel.sendMessage(':cd: Khong co bai hat nao trong hang doi.');
        },

        clear: msg => {
            bot.commands.stop(msg);
            bot.queue.clear();
            msg.channel.sendMessage(':cd: Da xoa ra khoi danh sach');
        },
        ...
        shuffle: msg => {
            bot.commands.stop(msg);
            bot.queue.shuffle();
            msg.channel.sendMessage(':arrows_counterclockwise: Queue xao tron');
        },
        ...
        volume: msg => {
            ...
            if(volume != '') {
                ...
                msg.channel.sendMessage(`:speaker: Tieng am thanh dang la: ${volume * 100}%`);
            } else
                msg.channel.sendMessage(`:speaker: Tieng am thanh dang la: ${bot.config.stream.volume * 100}%`);
        },

        repeat: msg => {
            bot.config.queue.repeat = !bot.config.queue.repeat;
            msg.channel.sendMessage(`Che do lap lai là: ${bot.config.queue.repeat ? 'on' : 'off'}`);
        },
            ...
            __.switch(action, {
                save: () => {
                    if(operation != undefined) {
                        ...
                        msg.channel.sendMessage(`Danh sach phat: "${operation}" da duoc them vao`);
                    }
                },
                load: () => {
                    if(operation != undefined) {
                        ...
                        msg.channel.sendMessage(`Danh sach phat: "${operation}" dang load`);
                    }
                },
                delete: () => {
                    if(operation != undefined) {
                        bot.playlist.delete(operation);
                        msg.channel.sendMessage(`Danh sach phat: "${operation}" da duoc xoa khoi danh sach phat`);
                    }
                },
                list: () => {
                    var playlists = bot.playlist.list();
                    playlists = __.map(playlists, (x, y) => '{0}. {1}'.format(y + 1, x));
                    msg.channel.sendMessage(playlists.length > 0 ? playlists.join('\n') : 'Khong co danh sach phat da luu');
                }
            });
        }
